...
public class DistributedToken implements Token, Serializable {

    private String masterToken;
    private Map<String, DistributedPageTokenValue> pageTokens;

    public DistributedToken(final String masterToken) {
        this(masterToken, new ConcurrentHashMap<>());
    }

    public DistributedToken(final String masterToken, final Pair<String, String> pageToken) {
        this(masterToken, toMap(pageToken));
    }

    private DistributedToken(final String masterToken, final Map<String, DistributedPageTokenValue> pageTokens) {
    ...
    public void setPageToken(final String uri, final String pageToken) {
        this.pageTokens.put(uri, DistributedPageTokenValue.from(pageToken));
    }

    @Override
    public String setPageTokenIfAbsent(final String uri, final Supplier<String> valueSupplier) {
        return this.pageTokens.computeIfAbsent(uri, k -> DistributedPageTokenValue.from(valueSupplier.get())).getValue();
    }
    ...
    public void setPageTokens(final Map<String, String> pageTokens) {
        this.pageTokens = pageTokens.entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        e -> DistributedPageTokenValue.from(e.getValue()),
                        (e1, e2) -> e2,
    ...
    public void rotateAllPageTokens(final Supplier<String> tokenValueSupplier) {
        this.pageTokens.entrySet().forEach(e -> e.setValue(DistributedPageTokenValue.from(tokenValueSupplier.get())));
    }

    @Override
    public void regenerateUsedPageToken(final String tokenFromRequest, final Supplier<String> tokenValueSupplier) {
        this.pageTokens.replaceAll((k, v) -> v.getValue().equals(tokenFromRequest) ? DistributedPageTokenValue.from(tokenValueSupplier.get()) : v);
    }

    private static Map<String, DistributedPageTokenValue> toMap(final Pair<String, String> pageToken) {
        final Map<String, DistributedPageTokenValue> pageTokens = new ConcurrentHashMap<>();
        pageTokens.put(pageToken.getKey(), DistributedPageTokenValue.from(pageToken.getValue()));
        return pageTokens;
    }
}
